    ...
    register: function(user, cb) {
        ...
        server.send({
            ...
            [{
                ...
                '<p><a href="http://' + sails.config.local.url + '/confirm?code="' + user.confirmationCode + '>Confirm Your Email</a></p> ' + 
                '<p>If the link above does not work, please copy and paste the following line in your browser URL address bar.</p>' +
                '<p>http://' + sails.config.local.url + '/confirm?code=' + user.confirmationCode + '</p>' +
                '</body>' + 
                '</html>', 
            alternative:true}]
